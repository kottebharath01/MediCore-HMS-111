...
const api = axios.create({
  baseURL: "https://medicore-hms-backend.onrender.com/api"
});
...
export const getAppointments = (s = "") =>
  api.get(`/appointments${s ? `?status=${s}` : ""}`);

export const createAppointment = (d) =>
  api.post("/appointments", d);

export const updateAppointment = (id, d) =>
  api.put(`/appointments/${id}`, d);

export const deleteAppointment = (id) =>
  api.delete(`/appointments/${id}`);

export const getRecords = (pid) =>
  api.get(`/records${pid ? `?patient_id=${pid}` : ""}`);
