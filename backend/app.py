...
app.config['JWT_ACCESS_TOKEN_EXPIRES'] = __import__('datetime').timedelta(hours=8)
...
class User(db.Model):
    ...
    patient_id = db.Column(db.Integer, db.ForeignKey('patients.id'), unique=True, nullable=True)
...
def can_access_patient(patient_id):
    user = current_user()
    return user and (user.role == 'admin' or user.patient_id == patient_id)


@app.before_request
def authenticate_api_requests():
    """All hospital API endpoints require a valid bearer token, except login/register."""
...
def seed_database():
    if User.query.filter_by(email='admin@medicore.local').first() is None:
        db.session.add(User(name='System Administrator', email='admin@medicore.local',
...
def register():
    data = request.get_json(silent=True) or {}
    for field in ('name', 'email', 'password', 'age', 'gender'):
        if not data.get(field):
            return jsonify({'message': f'{field} is required'}), 400
    email = data['email'].strip().lower()
    if User.query.filter_by(email=email).first():
        return jsonify({'message': 'An account with this email already exists'}), 409
    patient = Patient(name=data['name'], age=int(data['age']), gender=data['gender'], email=email,
                      phone=data.get('phone'), address=data.get('address'), blood_group=data.get('blood_group'))
    db.session.add(patient); db.session.flush()
    user = User(name=data['name'], email=email, password_hash=generate_password_hash(data['password']),
                role='user', patient_id=patient.id)
...
def dashboard():
    user = current_user()
    if user.role != 'admin':
        appointments = Appointment.query.filter_by(patient_id=user.patient_id).order_by(Appointment.date.desc()).all() if user.patient_id else []
        return jsonify({'stats': {
            'total_patients': 1 if user.patient_id else 0,
            'total_doctors': Doctor.query.count(),
            'today_appointments': sum(a.date == date.today() for a in appointments),
            'scheduled_appointments': sum(a.status == 'Scheduled' for a in appointments),
            'available_doctors': Doctor.query.filter_by(available=True).count(),
            'total_wards': 0, 'occupied_beds': 0, 'total_beds': 0, 'total_staff': 0,
        }, 'recent_appointments': [a.to_dict() for a in appointments[:5]]})
...
def get_patients():
    user = current_user()
    if user.role != 'admin':
        return jsonify([user.patient.to_dict()] if user.patient else [])
...
def get_patient(pid):
    if not can_access_patient(pid):
        return jsonify({'message': 'You may only view your own patient profile'}), 403
...
def create_appointment():
    ...
    if user.role != 'admin':
        if not user.patient_id:
            return jsonify({'message': 'Your account is not linked to a patient'}), 400
        d = {**d, 'patient_id': user.patient_id, 'status': 'Scheduled'}
    appt = Appointment(
...
def update_appointment(aid):
    ...
    user = current_user()
    if user.role != 'admin':
        if appt.patient_id != user.patient_id or set(d) - {'status'} or d.get('status') != 'Cancelled':
            return jsonify({'message': 'You may only cancel your own appointments'}), 403
